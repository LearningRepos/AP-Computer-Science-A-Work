...
public class Deck
{
    private List<Card> cardsList;    //contains all the cards in the deck
    ...
    public Deck(String[] ranks, String[] suits, int[] values)
    {
        ...
        for (int i = 0; i < ranks.length; i++) {
            for (int j = 0; j < suits.length; j++) {
                cardsList.add(new Card(ranks[i], suits[j], values[i]));
            }
        }
    ...
    public void shuffle()
    {
        ...
        {
            int index = rnd.nextInt(cardsList.size() - 1);
            Card a = cardsList.get(i);
            cardsList.set(i, cardsList.get(index));
            cardsList.set(index, a);
        }
    }
    ...
    public Card deal()
    {
        if (this.isEmpty())
            ...
            size--;
            return this.cardsList.get(size);
        }
    }
